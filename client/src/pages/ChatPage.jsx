...
const ChatPage = () => {
	...
	const { messages, getMessages, subscribeToMessages, unsubscribeFromMessages, sendMessage } = useMessageStore();
	...
	// Simple message sending function with page reload
	const handleSendMessage = async (messageContent) => {
		try {
			await sendMessage(messageContent);
			// Simply reload the page after sending
	...
	if (isLoadingMyMatches || !authUser) return <LoadingMessagesUI />;
	...
	return (
		<div className='flex flex-col h-screen bg-gray-100 bg-opacity-50'>
			...
			<div className='flex-grow flex flex-col p-4 md:p-6 lg:p-8 overflow-hidden max-w-4xl mx-auto w-full'>
				...
				<MessageInput match={match} handleSendMessage={handleSendMessage} />
			</div>
		</div>
	);
};
